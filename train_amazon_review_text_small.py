...
if __name__ == '__main__':
    ...
    train_dataloader, val_dataloader = model.get_data_loaders(review_dataset, 256, 0.8, num_workers=12)
    model.fit(torch.optim.Adam(model.parameters(), lr=0.001), train_dataloader, val_dataloader, epochs=8)
    # save the model
    torch.save(model.state_dict(), 'amazon_review_text_small.pth')
    # load the model
    model.load_state_dict(torch.load('amazon_review_text_small.pth'))
