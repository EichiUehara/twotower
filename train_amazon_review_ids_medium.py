...
if __name__ == '__main__':
    ...
    from dataset.amazon_review_ids.UserDataset import UserDataset
    from dataset.amazon_review_ids.ItemDataset import ItemDataset
    from dataset.amazon_review_ids.ReviewDataset import ReviewDataset
    ...
    train_dataloader, val_dataloader = model.get_data_loaders(review_dataset, 4096, 0.8, num_workers=12)
    model.fit(torch.optim.Adam(model.parameters(), lr=0.001), train_dataloader, val_dataloader, epochs=8)
    # save the model
    torch.save(model.state_dict(), 'amazon_review_ids_medium.pth')
    # load the model
    model.load_state_dict(torch.load('amazon_review_ids_medium.pth'))
